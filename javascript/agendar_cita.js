    ...
    try{
        ...
        $("#date_fecha").calendar({
            ...
            onSelect:function (event) {
                ...
                var formato_fecha="";
                formato_fecha += event.label.split('.')[2] + '-' + event.label.split('.')[1] + '-' + event.label.split('.')[0];
...
function eventos(){
    ...
    $("#btn_siguiente").on('click',function(e){
        ...
        if(VentanaActual==3){            
            ...
            $("#confirma_servicio").text($("#select_servicio option:selected").text());
            $("#confirma_proveedor").text($("#select_proveedor option:selected").text());
            ...
            if(Cliente_ID==0){
                ...
                $("#confirma_nombre").text($("#txt_nombre").val() + " " + $("#txt_apellido_p").val() + " " + $("#txt_apellido_m").val());
                ...
                $("#confirma_nombre").text(Datos_Cliente_Registrado.clientes_nombre  + " " + Datos_Cliente_Registrado.clientes_apellido_p + " " + Datos_Cliente_Registrado.clientes_apellido_m);
                ...
                if(Datos_Cliente_Registrado.clientes_telefono!="")
                    $("#confirma_telefono").text(Datos_Cliente_Registrado.clientes_telefono);

                if(Datos_Cliente_Registrado.clientes_direccion!="")
                    $("#confirma_domicilio").text(Datos_Cliente_Registrado.clientes_direccion);
...
function validar_campos(){
    ...
    if(VentanaActual==2){
        ...
        if($(".calendar .calendar-frame table tbody .selected").val()==undefined){
    ...
    if(VentanaActual==3){
        ...
        if(consultar_cliente_registrado()==false){
            ...
            if(obtener_campo_validacion("edad_cliente")=="true"){
                ...
                else if(parseInt($("#txt_edad").val(),10)<=0 || parseInt($("#txt_edad").val(),10)>=150)
        ...
        $.ajax({
            ...
            success: function (datos) {
                if (datos !== null) {
                    ...
                    $("#txt_descripcion_categoria").text(Servicio_Seleccionado.cls_descripcion_categoria);
                    $("#txt_descripcion_servicio").text(Servicio_Seleccionado.servicios_descripcion);
                    $("#txt_descripcion_costo").text("$ " + Servicio_Seleccionado.servicios_precio + ".00");
                    $("#txt_descripcion_duracion").text(Servicio_Seleccionado.servicios_duracion + " min.");
                ...
                if (datos !== null) {
                    ...
                    try{
                        ...
                        for(var i=0;i<datos.length;i++){
                            ...
                            if(datos[i].configuracion_nombre=="horario_lunes_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_martes_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_miercoles_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_jueves_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_viernes_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_sabado_inicio" && datos[i].configuracion_valor==""){
                            ...
                            if(datos[i].configuracion_nombre=="horario_domingo_inicio" && datos[i].configuracion_valor==""){
...
function generar_lista_horarios(formato_fecha){
    ...
    if(Array_Horarios.length!=0){
                ...
                success: function (datos) {
                    if (datos !== null) {
                        ...
                        try{
                            ...
                            alert('Error [consultar_descansos -> ajax, consultar_horas_ocupadas]' + ex);
                        }
                    }
                },
                error: function (datos) {
        ...
        for(var i=0;i<array_horas_disponibles.length;i++){
            ...
            $("#contenedor_horas").append('<button type="button" class="list-group-item list-group-item-action">'+ array_horas_disponibles[i] +'</button>');
        }
...
function consultar_cliente_registrado(){
    ...
    try{
        ...
        $.ajax({
            ...
            data: {"funcion" : "consultar_exacto", "obj_filtros": JSON.stringify(obj_filtros)},
            success: function(response)
            {                
...
function registrar_cliente(){
    ...
    try{
        ...
        obj_filtros.clientes_edad = $("#txt_edad").val()=="" ? 0 : parseInt($("#txt_edad").val(),10);
            ...
            {                
                ...
                try{
                    ...
                    if(envio_activo==true){
                        ...
                        var nombre_proveedor=$("#select_proveedor option:selected").text();
                        var nombre_servicio=$("#select_servicio option:selected").text();
                        var mensaje="";

                        if(Datos_Cliente_Registrado==null){

                            nombre_cliente = $("#txt_nombre").val() + " " + $("#txt_apellido_p").val() + " " + $("#txt_apellido_m").val();
                            ...
                            nombre_cliente = Datos_Cliente_Registrado.clientes_nombre + " " + Datos_Cliente_Registrado.clientes_apellido_p + " " + Datos_Cliente_Registrado.clientes_apellido_m;
                        }
                        ...
                        mensaje+="<span style='font-weight:bold'>Estimad@ " + nombre_proveedor + ".</span><br>";
                        mensaje+="Se ha agendado una nueva cita para tu servicio de " + nombre_servicio + "<br><br>";
                        ...
                        mensaje+="</ul>";
                        mensaje+="<br><br><br>Culaquier cambio que se realice en la cita favor de contactar al cliente y notificarle."
                            ...
                            {                
                                ...
                                try{
                                    ...
                                    if(jsonData.mensaje!="correcto"){
                                        ...
                                        Swal.fire({
                                            title: 'Error',
                                            text: 'Ha ocurrido un error enviando el correo, pero su cita ha sido registrada. Favor de contactar con administración para verificar: ' + jsonData.mensaje,
                                            icon: 'error',
                                            confirmButtonText: 'aceptar'              
                                        });
                                    }
